...
<div class="fieldcontain ${hasErrors(bean: auteurInstance, field: 'livres', 'error')} ">
	...
	<g:select name="livres" from="${biblio.Livre.list()}" multiple="multiple" optionKey="id" size="5" value="${auteurInstance?.livres*.id}" class="many-to-many"/>
</div>
